...
UINT16 	KeyFree[5];                                        //触摸空闲值存储，用于比较按键状态，如果采样值小于基准值表明按键按下
...
{	
	...
	for(i=KEY_FIRST;i<(KEY_LAST+1);i++)
  {
	  ...
	  TKEY_CTRL = (TKEY_CTRL & 0xF8 | i);                                     //设置采样通道 
		 while((TKEY_CTRL&bTKC_IF) == 0);
		 KeyFree[i] = TKEY_DAT&0x07F0;
//    while(j--)
//    {
...
//    }		
 //   KeyFree[i] = TKEY_DAT&0x07F0 + TmpSum/5;                                  //保存采样值 
		...
		printf("keyfree[%bd]=%d  tkey_dat=%x \n",i,KeyFree[i],TKEY_DAT);
	 TKEY_CTRL = TKEY_CTRL & 0xF8 | KEY_FIRST; 
	//	 KeyFree[i-KEY_FIRST] = TKEY_DAT&0x07F0 + TmpSum/5;
  }
...
void TouchKeyChannelQuery()
{
	...
	//	printf(" ch  %bd \n",ch);
	//printf(" KeyData =%d TKEY_DAT=%d KeyFree[%bd]=%d\n",KeyData,TKEY_DAT,ch,KeyFree[ch]);
                                                             //获取当前采样通道
	...
	//	printf(" TouchKey  %d \n",(KeyFree[ch] - KEY_ACT));
    if ( KeyData < (KeyFree[ch] - KEY_ACT) )                        //如条件满足，代表按键按下  
					  
    {			printf("keyfree[%bd]=%d KeyData[%bd] =%d TouchKey%bd=  %d \n",ch,KeyFree[ch],ch,KeyData,ch,(KeyFree[ch] - KeyData));
        KeyBuf=ch;    
        printf(" TouchKey Channel %bd \n",KeyBuf);                                                 //可以在此处进行按键动作处理或者置标志通知main进行处理
