...
//set按键
	...
	//		pm_12_24_alarm_display();
			...
			if(flag_display==0&&SET_Long_Flag==0)
			{
				...
				time.second=Read_addr_byte_ds1302(sec_r_addr);
				time.second = bcd_decimal(time.second);
			FD612_PotiveTNage(FD612_DIG5_ADDR,dis[time.second/10]);
			FD612_PotiveTNage(FD612_DIG6_ADDR,dis[time.second% 10]);										
			FD612_Refresh();	
			}
			else
			{
			FD612_PotiveTNage(FD612_DIG5_ADDR,0X00);
			FD612_PotiveTNage(FD612_DIG6_ADDR,0X00);										
			FD612_Refresh();			
			}			
...
//		}
		...
		if(flag_display==0&&SET_Long_Flag==0&&display_flag_hur_min==1)
		{
			FD612_PotiveTNage(FD612_DIG5_ADDR,dis[time.second/10]);
			FD612_PotiveTNage(FD612_DIG6_ADDR,dis[time.second% 10]);										
			FD612_Refresh();		
