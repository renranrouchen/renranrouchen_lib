...
s32 uart_update_init()
{
    ...
    if(!status)
    {
       ...
       uart_updata_param.custom |= BIT(14)|BIT(5)|BIT(3)|BIT(2);//接收超时�?�?允�??(5),接收�?�?允�??(3),发送中�?允�??(2)
...
/// 初�?�化必须把�??IO口�?�置输入，关�?上下拉，uboot必须对应�?�?
///IOMC2 BIT(29)-BIT(24):
///0x00-0x0f   PA[0:15]
///0x10-0x1f   PB[0:15]
///0x2b-0x2f   PC[11:15]
...
s32 EQ_uart_init()
{
    ...
    if(!status)
    {
       ...
       eq_param.custom = 0;
  //     UT1_CON = BIT(13) | BIT(12) |BIT(10)| BIT(6)| BIT(5) |BIT(3)|BIT(0);       //rx_ie , CLR RX PND  6:DMA接收�?�?
        //接收超时�?�?允�??(5),接收�?�?允�??(3),发送中�?允�??(2),BIT(7)清UT1_HRCNT
