    ...
    while(1)
    {
       ...
       // printf("\nmain_msg=========== %x\n",msg[0]);
        ...
        case SYS_EVENT_LGDEV_ONLINE:
            ...
            {
                if(((char)status == 'A'))       ///< SPIFLASH璁惧?噈ount
                {
                    puts("up\n");
...
//                    task_switch(BTSTACK_TASK_NAME, 0, SWITCH_SPEC_TASK);
        ...
        case MSG_ALM_ON:
            puts("\n\n\n\n------MSG_ALM_ON\n");
#if (PRODUCT_VERSION_KEYI == 1)
			timeSetting.alarmConfig.enterAlarmSettingFlag = true;
#endif
...
//            Post_msg_to_rcsp_Task(MSG_RSCP_CHANGE_WORDMODE_ACTION,0,0);
        ...
        case MSG_VOL_DOWN:
#if (PRODUCT_VERSION_KEYI == 1)
			volumnDisplayConfig.volumnDisplayFlag = true;
			volumnDisplayConfig.volumnDisplayDelay = 0;
#endif
            ...
            if (keymsg_task_name == FM_TASK_NAME)
            {
                printf("%s, MSG_VOL_DOWN, keymsg_task_name = %s, os_taskq_post_msg -> MSG_FM_RESET_VOL\n", __FUNCTION__, keymsg_task_name);
                os_taskq_post_msg(keymsg_task_name, 1, MSG_FM_RESET_VOL);
            }
            break;
...
#include "FD628_app.H"
        ...
        case MSG_HALF_SECOND: {
			RTC_TIME test;
			static u8 a = 0;
			memset(&test,0,sizeof(RTC_TIME));

			if (a++ < 1) {
				rtc_module_alarm_switch(0);
				//printf("flag %d",rtc_module_get_alarm_flag());
				printf("flag %d",get_alarm_flag()==0xaa?1:0);
			}
			break;
        	}
        }
