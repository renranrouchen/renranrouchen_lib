...
#if REC_EN
	rec_get_dac_data(rec_bt_api, (s16 *)read_buf, DAC_BUF_LEN);	//获取蓝牙和混响声�?
#if (REC_SOURCE == 1)//get data from dac
	rec_get_dac_data(rec_fm_api, (s16 *)read_buf, DAC_BUF_LEN);	//获取FM和混响声�?
	rec_get_dac_data(rec_aux_api, (s16 *)read_buf, DAC_BUF_LEN);	//获取AUX和混响声�?
#endif
