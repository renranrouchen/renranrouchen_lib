...
/* new feature add by zwxf */
u8 toneEndFlag = false;
...
static void tone_mutex_stop()
{
	...
	/* FIXME: can not post message!!! */
//	os_taskq_post_event(BTSTACK_TASK_NAME, 1, SYS_EVENT_PLAY_SEL_END);
	toneEndFlag = true;
}
...
/*   B T S T A C K _ K E Y _ H A N D L E R   */
/*-------------------------------------------------------------------------
*@param
*@Author			Jerry
...
static void btstack_key_handler(void *ptr,int *msg)
{
	...
	unsigned int displayBtPageCount = 0;
	unsigned char startDisplay = 0;
#if BT_TWS
    if(is_check_stereo_slave())
    {
        switch(msg[0])
