...
bool fm_module_set_fre(u8 mode)
{
    ...
    ret = (* fm_arr_set_fre[fm_mode_var->bAddr])(fm_mode_var->wFreq);
    if (ret)
        update_fm_freq(fm_mode_var->wFreq);
...
static void fm_info_init(void)
{
    ...
    fm_mode_var->wFreq = read_fm_freq();
    //printf("%s, read_fm_freq = %d\n", __FUNCTION__, fm_mode_var->wFreq);
    if (fm_mode_var->wFreq < MIN_FRE || fm_mode_var->wFreq > MAX_FRE)
    {
