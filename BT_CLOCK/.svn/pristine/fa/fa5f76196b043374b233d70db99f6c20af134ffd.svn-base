...
{
    return timeSetting.alarmConfig.alarmSwitch;
}
...
static void time_minus_process()
{
	switch (timeSetting.timeSettingPosition) {
		case TIME_YEAR_SETTING:
			if (timeSetting.currTime->year > 0) {
				timeSetting.currTime->year--;
			}
			break;
		case TIME_MONTH_SETTING:
...
{
	rtc_time_t temp;
	memset(&temp, 0, sizeof(rtc_time_t));
	temp.hour = timeSetting.alarmConfig.currAlarm->hour;
	temp.min  = timeSetting.alarmConfig.currAlarm->min;
	//rtc_module_write_alarm_datetime(&temp,0);
	update_alarm_time(&temp);
	printf("%s, update_alarm_time, %d-%d-%d, %d:%d:%d\n", __FUNCTION__, temp.year, temp.month, temp.day, temp.hour, temp.min, temp.second);
}
