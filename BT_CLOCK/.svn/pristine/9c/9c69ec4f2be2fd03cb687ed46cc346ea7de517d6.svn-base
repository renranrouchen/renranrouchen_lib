...
int music_decoder_init(MUSIC_OP_API **mapi_out)
{
    ...
    do
    {
        ...
        mapi->dop_api->io->father_name = "MusicTask";//解码结束的消�?返回给谁
...
/*----------------------------------------------------------------------------*/
/**@brief  文件解码前回调函数�?�理
   @param  m_op_api -- 解码模块指针，dev_let-逻辑盘�??
   @return void
   @note
*/
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
/**@brief  文件�?放总入�?
   @param  m_op_api -- 解码模块指针，dev_sel- 磁盘操作模式 dev_let-逻辑盘�?�，
            ...
            if(tmplgdev)
            {
                if(tmplgdev->lg_hdl->phydev_item == (void *)msg[1])
                {
                    ...
                    if((!mapi->dop_api->dec_api.phy_ops) || (DECODER_PAUSE != mapi->dop_api->dec_api.phy_ops->status))
                    {
                        ///< 解码设�?�非暂停状态，有解码自己报�?
                        break;
                    }///else的解码暂停状态，有下一�?消息SYS_EVENT_DEC_DEVICE_ERR处理
                }
                else
                {
                    ///<非解码�?��??
...
//                set_flac_sys_freq(0);//set_sys_clk
            ...
            {
                os_task_del_res_name(OS_TASK_SELF); 	//�?认可删除，�?�函数不再返�?
            }
...
#if MUSIC_AB_RPT_EN
		case MSG_MUSIC_AB_RPT:       ///<AB段循�?�?放�?�置
			puts("MUSIC_AB_RPT\n");
        ...
        case MSG_INPUT_TIMEOUT:
            ...
            UI_menu(MENU_MUSIC_MAIN);
            break;
        case SYS_EVENT_DEV_BEGIN_MOUNT:         ///<新�?��?�开始枚举，结束解码，防止出现新设�?�枚举慢，当前�?��?�播�?
            printf("new dev mount,stop  \n");   ///<一小�?�，又切换到新�?��??
            mapi->dop_api->dec_api.save_brk = 1;
            music_stop_decoder(mapi);
            break;
        case SYS_EVENT_DEV_MOUNT_ERR:       ///<新�?��?�枚举失败，继续当前设�?�播�?
...
*/
            ...
            switch(err)
            {
            case FILE_OP_ERR_INIT:              ///<文件选择器初始化错�??
            case FILE_OP_ERR_OP_HDL:            ///<文件选择器指针错�?
            case FILE_OP_ERR_LGDEV_NULL:        ///<没有设�??
            case FILE_OP_ERR_NO_FILE_ALLDEV:    ///<没有文件（所有�?��?�）
            ...
            case FILE_OP_ERR_OPEN_BPFILE:
                ...
                break;
            case FILE_OP_ERR_NUM:               ///<选择文件的序号出�?
                mapi->dop_api->file_num = 1;
                err = music_play_api(mapi,DEV_SEL_CUR,0,PLAY_SPEC_FILE,&(mapi->dop_api->file_num));
                break;
            case FILE_OP_ERR_LGDEV_MOUNT:
            case FILE_OP_ERR_NO_FILE_ONEDEV:    ///<当前选择的�?��?�没有文�?
                mapi->dop_api->file_num = 1;
            err = music_play_api(mapi,DEV_SEL_NEXT,0,PLAY_BREAK_POINT,&(mapi->dop_api->file_num));
                /* err = music_play_api(mapi,DEV_SEL_NEXT,0,PLAY_SPEC_FILE,&(mapi->dop_api->file_num)); */
                break;
            case FILE_OP_ERR_LGDEV_NO_FIND:     ///<没找到指定的逻辑设�??
                mapi->dop_api->file_num = 1;
                err = music_play_api(mapi,DEV_SEL_NEXT,0,PLAY_SPEC_FILE,&(mapi->dop_api->file_num));
                break;
