...
static void start_powerup_counting_down()
{
#include <FD612Drv.H>
	u8 count = 0;
	FD612_Init();
	for (count = 0; count < POWERUP_COUNTING_DOWN_MAX;count++) {
extern CONST INT8U DISP_POSITIVE_TABLE[];
#if 0
	    FD612_PotiveTNage(0,DISP_TAB[count]);
		FD612_PotiveTNage(2,DISP_TAB[count]);
		FD612_PotiveTNage(4,DISP_TAB[count]);
		FD612_PotiveTNage(6,DISP_TAB[count]);
#else
	    FD612_PotiveTNage(0,DISP_TAB[count]);
		FD612_PotiveTNage(1,DISP_TAB[count]);
		FD612_PotiveTNage(2,DISP_TAB[count]);
		FD612_PotiveTNage(3,DISP_TAB[count]);
#endif
		FD612_Refresh();
...
void  board_main(u32 cfg_addr, u32 addr, u32 res,u32 update_flag)
{
	...
	/* enter_sleep_mode(); */
#if POWERUP_COUNTING_DOWN
	start_powerup_counting_down();
#endif
    main_task_init();
    OSStart();
    irq_global_enable();
